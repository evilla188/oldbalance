...
const coworkerBubble = document.getElementById('coworkerBubble');
...
const coworkerLines = {
  inicio: [
    'Rodrigo: Hoy sí me propuse vender más que tú. A ver si puedes seguirme el ritmo.',
    'La Señora: Muchachos, primero atiendan bien y luego se pelean por las ventas.'
  ],
  bueno: [
    'Rodrigo: Esa estuvo buena. Primero entendiste lo que quería y después vendiste.',
    'La Señora: ¿Ves? El cliente se va contento y la caja también.'
  ],
  malo: [
    'Rodrigo: Creo que te fuiste demasiado rápido con la venta.',
    'La Señora: Mijo, resuelve primero. El cliente se acuerda de cómo lo hiciste sentir.'
  ]
};
...
const recurring = {
  Sofía: '¡Volví! La recomendación que me diste la vez pasada me encantó.',
  Rodrigo: 'Regresé porque sí me funcionó la ropa que me recomendaste.',
  'Doña Lupita': 'Mijo, tus tenis me salieron muy cómodos. Quiero otros.'
};
...
function showCoworkerLine(kind) {
  const list = coworkerLines[kind];
  coworkerBubble.textContent = list[Math.floor(Math.random() * list.length)];
  coworkerBubble.classList.remove('hidden');
  setTimeout(() => coworkerBubble.classList.add('hidden'), 2300);
}

function showEventBanner(event) {
  eventBanner.innerHTML = `<b>📣 ${event.name}</b><br>${event.text}`;
  eventBanner.classList.remove('hidden');
  setTimeout(() => eventBanner.classList.add('hidden'), 2800);
}
...
function openClient() {
  ...
  const returning = state.history.some(h => h.client === client.name && h.happy);

  clientNameEl.textContent = client.name;
  clientMoodEl.textContent = client.mood;
  clientStarsEl.textContent = returning ? '★★★★★ ↩' : '★★★★★';
  clientTextEl.textContent = returning && recurring[client.name] ? recurring[client.name] : client.intro;

  if (returning) {
    returnBadge.classList.remove('hidden');
    customerBubble.textContent = '¡Qué gusto verte otra vez!';
  } else {
...
function chooseAnswer(answer) {
  ...
  showCoworkerLine(answer.happy ? 'bueno' : 'malo');
  ...
  setTimeout(() => {
    ...
    guilleBubble.classList.add('hidden');
    state.clientIndex += 1;
    if (state.clientIndex === 4 || state.clientIndex === 7) applyEvent();
...
function finishDay() {
  ...
  document.getElementById('rRating').textContent = `${state.reputation.toFixed(1)}★`;
  document.getElementById('rRank').textContent = state.rank;
  document.getElementById('rReturns').textContent = state.returningClients;
...
function startGame() {
  ...
  returnBadge.classList.add('hidden');
  coworkerBubble.classList.add('hidden');
  guilleBubble.classList.add('hidden');
  customerBubble.classList.add('hidden');
  eventBanner.classList.add('hidden');
  updateHud();
  show(screens.game);
  showCoworkerLine('inicio');
  openClient();
}
